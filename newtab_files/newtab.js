...
function renderGroups() {
  ...
  board.groups.forEach((group, index) => {
    ...
    group.bookmarks.forEach(bk => {
      ...
      editBtn.title = 'Edit';
      editBtn.innerHTML = `<svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M3 17.25V21h3.75L17.81 9.94l-3.75-3.75L3 17.25z"/><path d="M14.06 4.94l3.75 3.75"/></svg>`;
      editBtn.addEventListener('click', (e) => {
        e.stopPropagation();
        const newTitle = prompt('Edit title:', bk.title);
        if (newTitle !== null) bk.title = newTitle.trim() || bk.title;
        const newUrl = prompt('Edit URL:', bk.url);
        if (newUrl !== null) bk.url = newUrl.trim() || bk.url;
        save(); renderGroups();
      });
      ...
      deleteBtn.innerHTML = `<svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14H6L5 6"/><path d="M10 11v6"/><path d="M14 11v6"/><path d="M9 6V4h6v2"/></svg>`;
  ...
  // Add group button: show at bottom of hovered column
  ...
  // Track mouse column and reposition add-group-card
  if (!state.editMode) {
    let _addGroupRafPending = false;
    grid.addEventListener('mousemove', (e) => {
      if (_addGroupRafPending) return;
      _addGroupRafPending = true;
      requestAnimationFrame(() => {
        _addGroupRafPending = false;
        const gridRect = grid.getBoundingClientRect();
        const mouseXInGrid = e.clientX - gridRect.left + grid.scrollLeft;

        // Find which column the mouse is in by checking group card positions
        const cards = Array.from(grid.querySelectorAll('.group-card'));
        if (!cards.length) return;

        // Group cards into columns by their X position
        const colMap = {};
        cards.forEach(card => {
          const cardX = parseInt(card.style.left || '0', 10);
          // round to nearest 10px to group cards in same column
          const colKey = Math.round(cardX / 10) * 10;
          if (!colMap[colKey]) colMap[colKey] = { x: cardX, cards: [] };
          colMap[colKey].cards.push(card);
        });

        // Find the closest column key to mouseXInGrid
        const colKeys = Object.keys(colMap).map(Number).sort((a, b) => a - b);
        let closestCol = colKeys[0];
        let minDist = Infinity;
        colKeys.forEach(k => {
          const colX = colMap[k].x;
          const dist = Math.abs(mouseXInGrid - (colX + GROUP_CARD_WIDTH_PX / 2));
          if (dist < minDist) { minDist = dist; closestCol = k; }
        });

        const col = colMap[closestCol];
        if (!col) return;

        // Find the bottom of the tallest card in this column
        let colBottom = GROUP_TOP_MARGIN_PX;
        col.cards.forEach(card => {
          const top = parseInt(card.style.top || '0', 10) || 0;
          const bottom = top + card.offsetHeight;
          if (bottom > colBottom) colBottom = bottom;
        });

        addCard.style.opacity = '1';
        addCard.style.left = `${col.x}px`;
...
function initDragAndDrop() {
  ...
  // Drop line indicator element (reused)
  let dropLine = document.getElementById('vt-drop-line');
  if (!dropLine) {
  ...
  bookmarkItems.forEach((item) => {
    ...
    item.addEventListener('dragover', (evt) => {
      ...
      showDropLine(item, before);
    });

    item.addEventListener('dragleave', (evt) => {
      // Only hide if leaving to something not within the same item
      if (!item.contains(evt.relatedTarget)) hideDropLine();
    });
  ...
  bookmarkLists.forEach((list) => {
    list.addEventListener('dragover', (evt) => {
      ...
      evt.preventDefault();
      evt.dataTransfer.dropEffect = 'move';
      // Show line below the last item in the list
      const items = list.querySelectorAll('.bookmark-item');
      if (items.length > 0) {
        const lastItem = items[items.length - 1];
        const rect = lastItem.getBoundingClientRect();
        // Only show at-end line if cursor is below all items
        if (evt.clientY > rect.bottom - 4) {
          showDropLine(lastItem, false);
...
document.getElementById('apply-wallpaper-btn').addEventListener('click', async () => {
  ...
  save(); await applyWallpaper(); closeModal('wallpaper-modal');
});
...
document.getElementById('wallpaper-file-input').addEventListener('change', async (e) => {
  ...
  reader.onload = async (ev) => {
    ...
    try {
      await dbSet('voidtab_wallpaper_local', dataUrl);
    } catch(err) {
      alert('Failed to save image. Please try a URL instead.');
      return;
    }
